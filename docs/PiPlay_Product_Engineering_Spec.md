...
Parses supported YouTube video/share/Shorts/embed/watch-playlist URLs and builds normalized watch or shell targets. Valid list IDs are retained; malformed IDs produce a non-blocking fallback reason. YouTube's 11-character embed keywords (`videoseries`, `live_stream`) are never video IDs, and a bare 11-letter word cased like a word (`programming`, `Documentary`) is a search rather than a video ID; IDs inside URLs are unaffected. (`YouTubeUrlHelperTests`.) `StartupArgumentPolicy` uses the same parser for command-line launch arguments; it accepts the first supported target verbatim and otherwise starts without one. (`AppStartupArgumentTests`, `StartupArgumentPolicyTests`.)
...
Schema `4`, sanitization/migration/recovery, atomic persistence. Corrupt `settings.json` is quarantined with an invariant-culture timestamp, or copied aside when it cannot be moved; a quarantine's age counts from when it was made, and quarantines older than 30 days are deleted. A `theme` block counts in any property case, as deserialization binds it, so only a file without one is seeded from the legacy Player fields. A read IO failure (lock, permissions, disk) is not corruption: SettingsService leaves the file untouched in place and refuses every save until a load succeeds or the user explicitly resets, so defaults never overwrite unread data. Only the read itself can mark a file unread; a file whose bytes were observed but did not parse is quarantined, not blocked, and the flag follows the file's full path. `Save` reports saved, refused-unread, or failed; the Source shows a one-time title-bar hint, **Settings not saved**, on the first refusal and never a modal for it. Reset app state saves its defaults through the same path, so the hint clears once the reset lifts the refusal. (`SettingsService`, `SettingsServiceTests`, `MainWindowLifecycleTests`.)
...
Capture Source state and target, acknowledge mute+pause, hide Source WebView, show the placeholder, and create/navigate the single Popout with the shared environment. A first item adopted from a playlist-only page starts at zero; an unrelated miniplayer/preview timestamp is never carried into it. When the Source was playing, the Popout's first playback sample decides one play nudge: a video that came up paused is played once, and one already playing is never nudged, so a later pause or the video's end stays paused. Failure restores Source visibility and captured playback state, applying the return rule for volume/mute/rate so an unknown launch mute state unmutes. (`MainWindow.xaml.cs`, `PlayerWindow.xaml.cs`, `PopoutTargetResolverTests`, `PopoutWindowCommandTests`.)
...
Return state is nullable; zero is a valid timestamp. Known live Popout state wins over launch fallback. Same-video return seeks; a different video/list navigates with context before replaying timestamp, play state, volume/mute, and rate where YouTube permits. A same-video decision is re-checked against the video the Source actually shows: if the Source moved, a known returned video navigates and an unknown one drives nothing; a same-video replay carries the identity of the video the Source showed at launch, and the Popout's reported id names the target only when the return navigates. The Popout pairs each playback sample with the navigation and return identity it was read for; a retarget or SPA identity change discards the earlier sample, so the new video returns with unknown time rather than the previous video's. Return restores Source placement/Pin and arms Auto de-dup before asynchronous replay. A return that lands while YouTube reports an ad applies volume/mute and the play/pause intent, then holds the seek and rate for a bounded wait (`12 × 500 ms`), writing them only if the page reports clear while the replay is still the pending one for the video the Source shows on the live core; after the bound, or if the Source moved, the page keeps its own position and native controls. One return transition is bounded by `20 s`: with a live core, a navigation whose completion never arrives, or a replay that hangs, releases the Source commands without replaying state. While the browser is recreating or failed, the transition holds the Source commands only as long as a replacement core is on its way: the failed state releases them at once, and the bound elapsing during a recreate releases them too; both keep the queued snapshot for the replacement core. (`PlayerReturnState`, `ReturnPolicy`, `ReturnReplayAdPolicy`, `ReturnPolicyTests`, `ReturnReplayAdPolicyTests`, `MainWindow.xaml.cs`, `PlayerWindow.xaml.cs`, `WpfRuntimeTests`, `MainWindowRecoveryTests`.)

## 15. WebView and navigation

### 15.2 Navigation and new windows

Source and Popout allow YouTube plus the supported Google sign-in/account hosts: `accounts`, `signin`, `myaccount`, and `consent` under `google.com`, a two-letter `google.<cc>`, or Google's `google.com.<cc>` / `google.co.<cc>` country domains. Unrelated HTTP(S) targets open externally or are blocked by surface policy. The allowlist is a navigation guardrail, not a malicious-page security boundary. (`NavigationPolicy`, `NavigationPolicyTests`.)
...
WebView2 process failures are classified once for both surfaces (ADR-0010): a renderer exit reloads the page on the live core, a browser-process exit recreates the Source control in place on the shared environment and returns to the last known page, and helper exits or an unresponsive renderer are logged only. One recovery runs at a time (a recreate coalesces every duplicate; a reload coalesces only another renderer exit), and after three consecutive automatic recoveries within `60 s` the Source shows the failed state, **The browser keeps failing**, with Retry, which resets the budget and takes the same recreate path. A reload that has not completed a navigation within `10 s` counts as another consecutive failure: it reloads again, or ends in the failed state once the budget is spent. A failed completion for a navigation that a newer one has already replaced settles nothing: the bound, the restart notice, and a queued replay wait for the newer navigation's own completion. The bound matters because a renderer that dies again while its reload is pending is coalesced, and no further event arrives. The Source panel names each state (**YouTube stopped responding**, **The browser component stopped**, **Restarting the browser**) and leaves on the first completed navigation of the live core; the failed state stays until Retry. The panel leads with Retry and offers the runtime download as a secondary way out (the download leads only when the runtime is missing); a passing notice hides the download and the Retry tooltip says it waits for the current recovery; the panel names a returned video or a chosen page that waits for the replacement core; the placeholder beneath it is disabled, and the browser surface is hidden while the panel shows because WPF cannot draw over the WebView2 child window. Back and Reload wait for a live core; the address box, Home, and profiles stay usable so the replacement core opens what the user chose. The Popout reloads once per renderer exit behind a one-line notice and closes on a browser-process exit, returning playback with its last polled sample; the return state carries the failure so the Source recreates before acting on it, whichever window heard of the exit first. A Popout that spends its budget on renderer exits also closes and returns, but without the failure flag: the shared browser and the Source's core are still alive. Any return in flight when the page renderer exits, or when its reload does not settle, ends immediately. A return that arrives while the browser is restarting or failed (the budget spent, or a restart that could not start the browser component), or that carries that failure flag, is queued: the returned video with its timestamp and playlist context becomes the page the replacement browser opens, and the snapshot (play state, volume/mute, rate) replays after that core's first successful same-video navigation, whether the recreate is automatic or started by Retry; a same-video replay that a browser-process exit or the failed state interrupts keeps its snapshot for the page the replacement core reopens. The queued snapshot is dropped when a Source navigation to anything but the pending video starts (whether or not it proceeds), when a browser-data clear starts, or when the window closes; an incoming link that arrives while the return transition holds is retained and applied when it releases (ADR-0009), and drops the snapshot only by that same rule. (`WebViewProcessFailurePolicy`, `WebViewProcessFailurePolicyTests`, `NavigationPolicy`, `NavigationPolicyTests`, `MainWindowRecoveryTests`, `MainWindowPolishTests`, `MainWindow.xaml.cs`, `PlayerWindow.xaml.cs`.)

An unhandled dispatcher exception is logged and recovered from behind at most one message box at a time; a repeat of the same fault signature (exception type plus throw site) within `10 s` of the last dialog is logged only. Out-of-memory, stack-overflow, access-violation, and SEH faults are logged, get one dialog, and are then left to terminate the process rather than marked handled. A fault raised once shutdown has started is logged without a dialog and does not block the exit. An unhandled exception that reaches the process and an unobserved task exception are logged only. (`DispatcherFaultPolicy`, `App`, `RuntimeFailurePolicyTests`.)
...
Popout chrome strip is `44 DIP` (`PlayerWindow.xaml`); its controls opt into chrome hit testing so the resize band never steals their top pixels. Double-clicking the drag handle expands or restores through the same path as the Expand button. Trusted primary mouse/pen drag on passive surface pixels begins only after `SystemParameters.MinimumHorizontalDragDistance` / `MinimumVerticalDragDistance`; below it the event remains a click. Interactive controls, ads, menus, captions, timelines, links, and overlays are excluded. Drag messages contain no coordinates. The host checks the logical primary button, which is the physical right button when the user has swapped mouse buttons. (`PlayerSurfaceDragProtocol`, `PlayerSurfaceScriptTests`, `BorderlessWindowHelperTests`, `PlayerWindow.xaml.cs`.)
...
Queue `4096` entries, batches `64 KiB`, failed-batch retention `512 KiB`, rotation near `1,000,000` bytes, files `logs\piplay.log` and one `.1` backup under the data root. Rotation re-reads the file length first, so a file another PiPlay process already rolled is not rolled again over its fresh backup. Timestamps use the invariant culture (`yyyy-MM-dd HH:mm:ss.fff`, Gregorian). Never log cookies, authorization headers, credential-bearing URLs, secrets, or unsanitized search text. (`LoggingService`, `LoggingServiceTests`, `AppPaths`.)
...
Every icon-only control has an accessible name and keyboard focus. Pin/transfer names state the next action. Code-built dialog inputs take their visible caption as their accessible name; a disabled primary action keeps a tooltip that says why; a checked toggle layers a tint over its surface and uses a heavier label as well as an accent cue; Returning and Clearing labels stay readable while disabled; the label of a filled button uses the ink picked by contrast for that fill. Destructive confirms (Delete, Clear browser data, Reset app state, Replace profile) style the confirm button as danger, default to Cancel, and say what is lost. `Ctrl+L` and `F6` focus the Source URL while Source commands are available.

Window shortcuts use only keys the WPF WebView2 control forwards as accelerator keys (Escape, function keys, Ctrl chords), so they also work while the page has focus; plain keys stay with YouTube. Source: `Ctrl+Shift+P` runs the Pop out video / Bring video back button, and `Ctrl+T` the Source Pin, each only while that control is enabled. Popout: `F11` expands or restores, `Esc` restores an expanded Popout (and is left to the page otherwise), `Ctrl+T` pins, and `Ctrl+W` or `Ctrl+Shift+P` brings the video back. A press acts once: auto-repeat, which WebView2 forwards without a repeat flag, is swallowed until a key-up or deactivation. Tooltips and the Popout's context menu name the gestures, and accessible names stay the plain action. The Popout window title names the video from an https YouTube document title without YouTube's notification count and site suffix, falls back to `PiPlay Video Popout` elsewhere, and is never logged. (`KeyboardShortcutPolicy`, `KeyboardShortcutInput`, `PopoutTitlePolicy`, `MainWindow.xaml`, `PlayerWindow.xaml`, `MainWindow.xaml.cs`, `PlayerWindow.xaml.cs`, `Prompt.cs`, `KeyboardShortcutPolicyTests`, `PopoutTitlePolicyTests`, `PopoutWindowCommandTests`, `MainWindowPolishTests`, `PromptPolishTests`, `ThemePolishTests`, UI tests.)
